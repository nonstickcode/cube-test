...
const PasswordStrengthMeter: React.FC = () => {
  ...
  const calculatePasswordStrength = (password: string): (Weakness | undefined)[] => {
    const weaknesses: (Weakness | undefined)[] = [];
    weaknesses.push(lengthWeakness(password));
    weaknesses.push(characterTypeWeakness(password, /[a-z]/g, "lowercase characters"));
    weaknesses.push(characterTypeWeakness(password, /[A-Z]/g, "uppercase characters"));
    weaknesses.push(characterTypeWeakness(password, /[0-9]/g, "numbers"));
    weaknesses.push(characterTypeWeakness(password, /[^0-9a-zA-Z\s]/g, "special characters"));
  ...
  const characterTypeWeakness = (password: string, regex: RegExp, type: string): Weakness | undefined => {
  ...
  return (
    <div id='password-strength-meter-container' className='container'>
      ...
      <div className="strength-meter" style={{ '--strength': `${strength}%` } as React.CSSProperties}></div>
      <input
